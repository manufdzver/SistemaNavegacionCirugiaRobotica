...
void escribePos()
{
	...
	*/
	std::fstream my_file;
	my_file.open("C:/Users/manuf/OneDrive/Escritorio/SistemaNavegacionCirugiaRobotica/Files/my_file.txt", std::ios::out);
	if (!my_file) {
...
/*
void consumeCallback(const AMQP::Message& message, uint64_t deliveryTag, bool redelivered)
{
...
void rabbitFunc()
{
	...
	amqp_exchange_declare(conn, 1, exchange_name, amqp_cstring_bytes("fanout"), 0, 0, 0, 0, amqp_empty_table);

	amqp_bytes_t message_bytes = amqp_cstring_bytes(message.c_str());
	amqp_basic_publish(conn, 1, exchange_name, amqp_empty_bytes, 0, 0, NULL, message_bytes);
